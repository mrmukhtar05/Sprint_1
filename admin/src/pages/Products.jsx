          ...
          <form onSubmit={save} className="admin-form">
            <div className="admin-form-grid ">
              ...
              <select className="admin-input" value={form.gender} onChange={e => setForm({ ...form, gender: e.target.value })}>
                {GENDERS.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select>
              <select className="admin-input" value={form.category}
                onChange={e => setForm({ ...form, category: e.target.value })}><option value="">Select category</option>
            ...
            <div><label className="admin-upload-label">SIZE</label><div className="admin-filter-row">{SIZES.map(s => <button type="button" key={s}
              className={`admin-filter-chip ${form.sizes.includes(s) ? "active" : ""}`} onClick={() => toggleArray("sizes", s)}>{s}</button>)}</div></div>

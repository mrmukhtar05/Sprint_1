...
const getAdminProducts = asyncHandler(async (req, res) => {
  const { page = 1, limit = 20, search = "", gender, category } = req.query;
  ...
  const [products, total] = await Promise.all([
    Product.find(query).select("name slug category images price discountPrice stock sku gender sizes colors isFeatured isActive createdAt").populate("category", "name image").sort({ createdAt: -1 }).skip((pageNum - 1) * limitNum).limit(limitNum).lean(),
    Product.countDocuments(query),
  ]);
  res.json({ success: true, count: products.length, total, page: pageNum, pages: Math.ceil(total / limitNum), products });
});
